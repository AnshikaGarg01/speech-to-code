...
export class SpeechToTextComponent implements AfterViewInit {
    ...
    this.recognition.onresult = (event) => {
      for (let i = event.resultIndex; i < event.results.length; i++) {
        if (event.results[i].isFinal) {
          this.wordList.push(event.results[i][0].transcript.toLowerCase());
  ...
  cleanupInput(input) {
    ...
    const numberPart =
      input.match(/\d+/) == null ? null : input.match(/\d+/)[0];
  ...
  codeSnippetsMatch() {
    const transcript = this.wordList[this.wordList.length - 1];
    console.log("----", this.wordList, transcript);
